...
public class Predict {
	...
	public static String[] predictVm(String[] ecsContent, String[] inputContent){
		...
		int pDays = IntervalDays + preDays;
		/**=====预测阶段===**/
		/**************************提交注释******************************/
		...
		for(int i=0 ; i<fNum ; i++){  //预测是按照fNameArray的顺序来预测每个虚拟机的预测结果
			...
			double[] smooth1Array = ExpSmoothPredict.GetOnceSmoothArray(detectArray, 0.5, detectArray[0]);
			
			double[] smooth2Array = ExpSmoothPredict.GetOnceSmoothArray(smooth1Array, 0.6, detectArray[0]);
			
			double[] predictArray = ARGradientPredict.GetPredictFArray(smooth2Array, pDays, 1);	//AR模型获取预测数据 
						
			
			//从预测数据中切割出input需要的预测数据
			/*double[] preArray = new double[preDays];
			for(int j=0; j<preDays; j++){
				preArray[j] = predictArray[j+IntervalDays];
			}*/								

...
public class GA1Distribute {
	 ...
	 * @return
	 */
	public static double[] GetFitness(int[][] population, int nCPU, int nMEM){
	 ...
	 * @return
	 */
	public static int[] GenAlg(int[] person, int n, int nCPU, int nMEM, double maxFitness, int maxEpoch){
		int k = person.length;
		...
		while(!end){
			...
			double[] parentfitArray = GetFitness(parentPopulation, nCPU, nMEM);			
			...
			double[] tempChildFitArray = GetFitness(tempChildPopulation, nCPU, nMEM);
			...
			s++;
			//System.out.println("第"+s+"代的最优个数为："+1/tempFitArray[0]);
			if(s>=maxEpoch || tempFitArray[0]>=maxFitness){
				System.out.println("最优服务器个数："+1/tempFitArray[0]);
				end = true;
			}
	...
	/**
 	...
 	public static String[] distribute(String[] fName, int[] fData, int nCPU, int nMEM){
 		...
 		double maxFitness = 1.0/Math.max((int)(fsumCPU/nCPU+1), (int)(fsumMEM/nMEM+1));
 		
 		int[] optPerson = GenAlg(person, 10, nCPU, nMEM, maxFitness, 500);
 		...
 		System.out.println("最少的服务器个数："+Math.max((int)(fsumCPU/nCPU+1), (int)(fsumMEM/nMEM+1)));
 	...
 	/*public static void main(String[] args) {
		...
		String[] result = distribute(fName, fData, 56, 128);

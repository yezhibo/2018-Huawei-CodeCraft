...
public class DataFitPredict {
	...
	public static double[] gradientDescent(double[] dataArray, int p){
		...
		double gama = 0.001;   //初始化迭代步长
		
		double epsilon = 0.0001; //初始化结束阈值
		
		while(true){      //开始利用梯度下降算法估计残差最小的AR模型参数
			...
			//System.out.println("第"+k+"次迭代误差值："+Math.sqrt(err));
			...
			*//**如果距离小于给定值, 结束梯度下降算法**//*
			...
			if(k==2000){     //如果迭代次数超过2000次,则将此时的误差赋给结束阈值
				epsilon = Math.sqrt(distance);
			}				
		}					
	}*/
	...
	public static double[] gradientDescent(double[] normalArray, int p){
		...
		for(int i=0 ; i<p ; i++){
			optA[i][0] = 0/*Math.random()*/;  //参数初始值产生 -0.1 到0.1之间的随机数
		}
	...
	public static List<double[]> GetARpara(double[] normalArray){
		 ...
		 * 思路：残差最小的即为最优的阶数
		 */
		for(int p=n/4 ; p>9 ; p--){        //通过遍历求出最优的AR阶数,针对于差分数据
	...
	public static double[] GetPredictArray(double[] detectArray, int pDays){	
		...
		/**数据预处理**/			
		double[] diffarray = DataPreprocess.SevenDiff(detectArray);
